...
client.on('ready', async () => {
 ...
 client.user.setActivity(`Kutay 🖤 Owen`, { type:'PLAYING' })
...
client.on("message", message => {
  if (message.content.startsWith('.rolver')) {
    if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('yetki yok' ) // yetki yok
...
client.on("message", message => {
  ...
  client.on('message', message => {
    if (message.content.startsWith('.oylama')) {
      ...
      const botoylama = args.join(" ")
      if (!message.member.hasPermission("ADMINISTRATOR")) return message.reply('Oylama Yapmak icin YONETICI Rolune Sahip Olman Gerekir');
      const embed = new MessageEmbed()
      .setTitle(botoylama)
      .setDescription('Kutay Development')
      .setFooter('OYLAMA');
